    ...
    def test_prediction_endpoint(self):
        ...
        try:
            ...
            response = self.session.post(
                f"{self.base_url}/predict", json=test_data, timeout=self.timeout
            )
            ...
            logger.info(
                f"✅ Prediction endpoint test passed (prediction: {prediction})"
            )
            return True
    ...
    def test_api_response_time(self):
        ...
        try:
            ...
            if response_time > max_response_time:
                raise AssertionError(
                    f"Response time too slow: {response_time:.2f}s > {max_response_time}s"
                )
