            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if attempt < max_retries - 1:
                        logger.info(
                            f"API not ready, retrying in {backoff_delay}s... (attempt {attempt + 1}/{max_retries})"
                        )
        ...
        try:
            ...
            for test_case in test_cases:
                response = self.session.post(
                    f"{self.base_url}/predict", json=test_case["data"], timeout=10
                )

                if response.status_code != 200:
                    raise AssertionError(
                        f"Prediction failed for {test_case['name']}: {response.status_code}"
                    )
                ...
                if not isinstance(prediction, (int, float)) or prediction < 0:
                    raise AssertionError(
                        f"Invalid prediction for {test_case['name']}: {prediction}"
                    )

                logger.info(
                    f"✅ Prediction test '{test_case['name']}' passed (prediction: {prediction:.1f})"
                )
    ...
    def check_performance_metrics(self):
        ...
        try:
            ...
            for i in range(num_requests):
                ...
                response = self.session.post(
                    f"{self.base_url}/predict", json=test_data, timeout=10
                )
            ...
            if avg_response_time > max_avg_time:
                raise AssertionError(
                    f"Average response time too slow: {avg_response_time:.2f}s > {max_avg_time}s"
                )

            if max_response_time > max_single_time:
                raise AssertionError(
                    f"Maximum response time too slow: {max_response_time:.2f}s > {max_single_time}s"
                )

            logger.info(
                f"✅ Performance check passed (avg: {avg_response_time:.2f}s, max: {max_response_time:.2f}s)"
            )
            return True
    ...
    def check_error_handling(self):
        ...
        try:
            ...
            for case in invalid_cases:
                response = self.session.post(
                    f"{self.base_url}/predict", json=case["data"], timeout=10
                )

                if response.status_code != case["expected_status"]:
                    logger.warning(
                        f"⚠️  Unexpected status for {case['name']}: {response.status_code} (expected {case['expected_status']})"
                    )
                else:
        ...
        try:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    logger.warning(
                        f"⚠️  Metrics endpoint returned {response.status_code}"
                    )
            except:
...
def main():
    ...
    if not api_url:
        logger.warning(
            "API_BASE_URL environment variable not set, using default: http://localhost:8000"
        )
