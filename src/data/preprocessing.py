...
class DataPreprocessor:
    ...
    def validate_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if missing_values.any():
            logger.warning(
                f"Missing values found: {missing_values[missing_values > 0]}"
            )
        ...
        for feature in weather_features:
            if feature in df.columns:
                ...
                if out_of_range > 0:
                    logger.warning(
                        f"Found {out_of_range} out-of-range values in {feature}"
                    )
    ...
    def prepare_features(
        ...
        available_features.extend(
            [col for col in engineered_features if col in df.columns]
        )
    ...
    def split_data(
        self, df: pd.DataFrame, test_size: float = None, random_state: int = None
